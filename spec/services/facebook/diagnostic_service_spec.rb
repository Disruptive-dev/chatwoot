...
describe Facebook::DiagnosticService do
  ...
  before do
    ...
    allow(fb_object).to receive(:get_object).with(facebook_channel.page_id, fields: 'id,name').and_return({ 'id' => facebook_channel.page_id, 'name' => 'Test Page' })
    allow(fb_object).to receive(:get_object).with('psid-123', fields: Facebook::ProfileFetcher::PROFILE_FIELDS).and_return({ 'name' => 'Jane Doe' })
